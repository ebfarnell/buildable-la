...
async function validateProperty() {
  ...
  const parcelUrl =
    'https://public.gis.lacounty.gov/public/rest/services/Assessor/Assessor_Parcel_Public/MapServer/0';
  ...
  console.log(
    `  Front: ${setbacks.frontFt} ft, Sides: ${setbacks.sideFt} ft, Rear: ${setbacks.rearFt} ft`,
  );
  ...
  console.log(
    `Difference in buildable: ${buildDiff.toFixed(0)} sqft (${((buildDiff / 3994) * 100).toFixed(1)}%)`,
  );
